...
impl Database {
    ...
    pub async fn get_folders(&self, account_id: &str) -> Result<Vec<FolderRecord>> {
        let conn = Arc::clone(&self.conn);
        let account_id = account_id.to_string();
        tokio::task::spawn_blocking(move || {
            let conn = conn.lock().unwrap();
            let mut stmt = conn.prepare("SELECT id, name, included_chat_types, excluded_chat_types, included_chat_ids, excluded_chat_ids, sort_order FROM chat_folders WHERE account_id = ? ORDER BY sort_order").context("prepare")?;
    ...
    pub async fn save_folder(&self, account_id: &str, folder: &FolderRecord) -> Result<()> {
        let conn = Arc::clone(&self.conn);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let mut stmt = conn.prepare("INSERT INTO chat_folders (id, account_id, name, included_chat_types, excluded_chat_types, included_chat_ids, excluded_chat_ids, sort_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id, account_id) DO UPDATE SET name=excluded.name, included_chat_types=excluded.included_chat_types, excluded_chat_types=excluded.excluded_chat_types, included_chat_ids=excluded.included_chat_ids, excluded_chat_ids=excluded.excluded_chat_ids, sort_order=excluded.sort_order, updated_at=excluded.updated_at").context("prepare")?;
    ...
    pub async fn delete_folder(&self, account_id: &str, id: &str) -> Result<()> {
        let conn = Arc::clone(&self.conn);
        let id = id.to_string();
        let account_id = account_id.to_string();
        tokio::task::spawn_blocking(move || {
    ...
    pub async fn get_tabs(&self, account_id: &str) -> Result<Vec<TabRecord>> {
        let conn = Arc::clone(&self.conn);
        let account_id = account_id.to_string();
        tokio::task::spawn_blocking(move || {
            let conn = conn.lock().unwrap();
            let mut stmt = conn.prepare("SELECT id, visible, sort_order FROM chat_tabs WHERE account_id = ? ORDER BY sort_order").context("prepare")?;
            stmt.bind((1, account_id.as_str())).unwrap();
            let mut result = Vec::new();
            while let Ok(sqlite::State::Row) = stmt.next() {
    ...
    pub async fn save_tabs(&self, account_id: &str, tabs: &[TabRecord]) -> Result<()> {
        let conn = Arc::clone(&self.conn);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            for tab in &tabs {
                let mut stmt = conn.prepare("INSERT INTO chat_tabs (id, account_id, visible, sort_order) VALUES (?, ?, ?, ?)").context("prepare")?;
